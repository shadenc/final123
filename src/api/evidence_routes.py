"""Flask route registration for evidence_api (split from create_app for Sonar cognitive complexity)."""
...
from src.api.evidence_api import (
    ...
    REINVESTED_CSV_RELPATH,
    RESULTS_JSON_RELPATH,
    ...
    _run_combined_pipeline_task,
    _run_net_profit_background_task,
    _run_pdfs_pipeline_task,
    resolve_evidence_screenshot_paths,
...
class EvidenceRouteContext:
    """Paths shared by Flask handlers (was closure vars in create_app)."""

    __slots__ = ("root", "screenshots_dir", "results_file", "metadata_file", "reinvested_csv")

    def __init__(self, project_root: Path) -> None:
        self.root = project_root
        self.screenshots_dir = project_root / SCREENSHOTS_RELPATH
        self.results_file = project_root / RESULTS_JSON_RELPATH
        self.metadata_file = self.screenshots_dir / "evidence_metadata.json"
        self.reinvested_csv = project_root / REINVESTED_CSV_RELPATH
...
def register_evidence_api_routes(app: Flask, ctx: EvidenceRouteContext) -> None:
        ...
        def get_evidence_screenshot(company_symbol):
            ...
            try:
                ...
                screenshot_files = resolve_evidence_screenshot_paths(ctx.screenshots_dir, company_symbol, quarter)
        ...
        def get_extractions():
            ...
            try:
                ...
                with open(ctx.results_file, 'r', encoding='utf-8') as f:
                    results = json.load(f)

                # Load evidence metadata if available
                evidence_metadata = {}
                if ctx.metadata_file.exists():
                    with open(ctx.metadata_file, 'r', encoding='utf-8') as f:
        ...
        def get_evidence_metadata():
            ...
            """
            try:
                if not ctx.metadata_file.exists():
        ...
        def get_retained_earnings_flow_csv():
            ...
            try:
                csv_path = ctx.root / FLOW_CSV_RELPATH
        ...
        def get_reinvested_earnings_results():
            ...
            """
            try:
                if not ctx.reinvested_csv.exists():
                    logger.warning(f"CSV file not found: {ctx.reinvested_csv}")
                    return jsonify({"error": "Data not available"}), 404

                return send_file(
                    str(ctx.reinvested_csv), 
                    mimetype='text/csv',
                    as_attachment=True,
        ...
        def refresh_data():
            ...
            try:
                ...
                try:
                    ...
                    scraper_script = ctx.root / "src/scrapers/ownership.py"
        ...
        def health_check():
            ...
            return jsonify({
                "status": "healthy",
                "screenshots_dir": str(ctx.screenshots_dir),
                "screenshots_available": ctx.screenshots_dir.exists()
            })
        ...
        def pdfs_status():
            progress_file = ctx.root / RUNTIME_PDFS_PROGRESS_JSON
        ...
        def net_profit_status():
            progress_file = ctx.root / RUNTIME_NET_PROGRESS_JSON
            if progress_file.exists():
                try:
                    with open(progress_file, 'r', encoding='utf-8') as f:
                        return jsonify(json.load(f))
        ...
        def stop_pdfs_pipeline():
            flag = ctx.root / RUNTIME_STOP_PDFS_FLAG
            try:
                ...
                try:
                    progress_path = ctx.root / RUNTIME_PDFS_PROGRESS_JSON
        ...
        def stop_net_profit():
            flag = ctx.root / RUNTIME_STOP_NET_FLAG
            try:
                flag.parent.mkdir(parents=True, exist_ok=True)
                flag.write_text('stop', encoding='utf-8')
                return jsonify({"status": "accepted"})
        ...
        def correct_retained_earnings():
            ...
            results_file = ctx.root / "data/results/retained_earnings_results.json"
            try:
            ...
            try:
                subprocess.run([sys.executable, str(ctx.root / SCRIPT_CALCULATE_REINVESTED)], check=True)
            except Exception as e:
            ...
            csv_file = ctx.root / "data/results/reinvested_earnings_results.csv"
            try:
        ...
        def update_ownership_data():
            ...
            try:
                ...
                ownership_script = ctx.root / "src/scrapers/ownership.py"
        ...
        def get_net_profit():
            ...
            try:
                net_profit_file = ctx.root / QUARTERLY_NET_PROFIT_RELPATH
        ...
        def get_quarter_evidence_mapping(company_symbol):
            ...
            try:
                ...
                for quarter_type, info in mapping_info['evidence_mapping'].items():
                    ...
                    screenshot_files = list(ctx.screenshots_dir.glob(pattern))
        ...
        def get_previous_quarter_evidence(company_symbol):
            ...
            try:
                ...
                screenshot_files = list(ctx.screenshots_dir.glob(previous_quarter_pattern))
        ...
        def trigger_quarterly_archive():
            ...
            try:
                ...
                run_quarterly_refresh_and_archive(ctx.root)
